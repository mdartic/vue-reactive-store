...
import { VRSStore, VRSPlugin, VRSComputed, VRSStateTree } from '../types'
...
const vrsPluginDevtools = (vrsStore: VRSStoreForDevtools): VRSPlugin => {
  if (!devtoolHook) {
    console.warn(
      '[vrs-plugin-devtools] vue-devtools is not installed. Please install before trying to use vrs-plugin-devtools'
    )
    return {}
  }
  ...
  vrsStore.flushStoreModules = () => {
    console.warn(
      '[vrs-plugin-devtools]flush vrsStore modules is not implemented (asked by vue-devtools)'
    )
  }
  vrsStore.registerModule = (module: string) => {
    console.warn(
      `[vrs-plugin-devtools]registerModule is not implemented (asked by vue-devtools for module '${module}')`
    )
  }
  vrsStore.unregisterModule = (module: string) => {
    console.warn(
      `[vrs-plugin-devtools]unregisterModule is not implemented (asked by vue-devtools for module '${module}')`
    )
  }
   ...
   */
  function replaceState(targetStore: VRSStore, targetState: VRSStateTree) {
    if (!targetState) {
      console.warn("[vrs-plugin-devtools] state is null. can't replace a state with null")
      return
    }
    Object.keys(targetState).forEach(k => {
      if (targetStore.modules && targetStore.modules[k]) {
        return replaceState(targetStore.modules[k], targetState[k] as VRSStateTree)
      }
      targetStore.state![k] = targetState[k]
    })
  }

  vrsStore.replaceState = (targetState: VRSStateTree) => {
    console.info(
      '[vrs-plugin-devtools] replacing state... (asked by vue-devtools) ',
   ...
   */
  devtoolHook.on('vuex:travel-to-state', (targetState: VRSStateTree) => {
      ...
      after(rootStore, stateProperty, newValue) {
        ...
        vrsStore._mutations[stateProperty] = (payload: any) => {
          ...
          for (i; i < arrayPaths.length; i++) {
            currentStateObject = (currentStateObject[arrayPaths[i]] as VRSStateTree) || {}
